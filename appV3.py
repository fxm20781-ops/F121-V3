...
if uploaded_file is not None:
    try:
        ...
        if df_clean.empty:
            ...
            # 【關鍵修改】將 F121 outlet temperature 移至側邊欄作為輸入
            input_out_temp = st.sidebar.slider(
                "F121 outlet temperature 出路溫度 (°C)", 
                bounds_config['out_temp_min'], 
                bounds_config['out_temp_max'], 
                (bounds_config['out_temp_min'] + bounds_config['out_temp_max'])/2
            )
            ...
            if st.button("🚀 開始計算最低天然氣消耗控制策略", type="primary"):
                with st.spinner("正在根據指定溫度全局搜索最省能的操作組合..."):
